...
{{
  "resource": {{
    "type": "book" or "video" or "mix",
    "title": "resource title",
    "url": "https://..."
  }},
  "note": null
}}
- "resource" MUST be an object with type, title, url keys. NEVER a plain string.
...
""".strip()
    ...
    try:
        pick = GeminiResourcePick.model_validate(raw)
    except Exception:
        return GeminiResourcePick(resource=None, note="Gemini returned invalid resource structure.")
...
""".strip()
    ...
    for course in raw.get("courses", []):
        ...
        if not isinstance(plan, list):
            plan = []
        plan = [str(x).strip() for x in plan if str(x).strip()][:5]
        while len(plan) < 5:
